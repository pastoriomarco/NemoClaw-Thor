...
|---|---|---|
| **Entrpi DS4 v0.5.5 + Thor profile** | **healthy; repaired selector, 512K allocation, and queued concurrency validated (2026-08-05)** | Isolated opt-in Docker service on `:8050`, not wired into ManyForge `:8000`. Source pin `2e9799073e08ea8f89eb1e72c47328ee6d90c6e8`; CUDA 13.0.1 bases are digest-pinned and compilation is strictly `sm_110`. Uses the 0731 base + matching DSpark drafter only (`--no-mtp`). The default `runtime-thor-v055` image uses Entrpi's repaired streaming top-512 selector: 513 Thor stream-vs-tree comparisons through 105K were exact, normal 105K prefill reached 411.6 tok/s, and exact 244,518-token retrieval reached 348.2 tok/s. The profile remains 512K context, two banks, 4K effective chunks, 12 GiB VMM budget, and an 8 GiB live-memory floor. Five two-caller and two four-caller waves completed without fallback, OOM, Xid, or restart; four callers queue over two banks. A small v0.5.5 server patch preserves `finish_reason=length` for continuous tool-call budget cuts. Prior exact retrieval at 479,817 tokens remains the near-limit capacity proof. Runbook: [`serving/docs/DS4-ON-THOR.md`](serving/docs/DS4-ON-THOR.md). |
