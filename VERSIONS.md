...
|---|---|---|
| **Entrpi DS4 v0.5.1** | **healthy on Thor at 256K (2026-08-01)** | Isolated opt-in Docker service: `serving/start-ds4.sh`, exposed on `192.168.1.136:8050` for the LAN client and not yet wired into the ManyForge `:8000` proxy. Source pin `161b23609ab8a928246d268cec61101007f678b3`; two-stage CUDA 13.0.1 image is pinned by digest; compile target is strictly `sm_110`. Uses the DeepSeek-V4-Flash-0731 base + matching DSpark drafter only (`--no-mtp`). The v0.5.1 streaming top-512 indexer triggers a reproducible Xid 13 on `sm_110`; Entrpi's output-equivalent `DS4_CUDA_NO_TOPK_STREAM` escape fixes it. Exact-needle requests passed at 84,797, 126,215, and 247,065 prompt tokens with a 262,144-token window. Runbook: [`serving/docs/DS4-ON-THOR.md`](serving/docs/DS4-ON-THOR.md). |
