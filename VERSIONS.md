...
|---|---|---|
| **Entrpi DS4 v0.5.4 + Thor profile** | **healthy; 512K depth and two-bank stability validated (2026-08-04)** | Isolated opt-in Docker service on `:8050`, not wired into ManyForge `:8000`. Source pin `215af2f1245324bcebf9a69a498eff79275aac8e`; CUDA 13.0.1 bases are digest-pinned and compilation is strictly `sm_110`. Uses the 0731 base + matching DSpark drafter only (`--no-mtp`). The default `runtime-thor-topk` image replaces Entrpi's Xid-13-prone atomic selector with a verified bounded, atomics-free exact top-512 merge; 1,025 dual-run comparisons through 244K matched the safe tree and controlled 105K prefill improved 10.6%. The normal profile is 512K context, two banks, 4K effective chunks, 12 GiB VMM budget, and an 8 GiB live-memory floor; five consecutive two-request waves passed cleanly. Exact retrieval passed at 479,817 prompt tokens (249.9 tok/s prefill, 32-minute cold TTFT). A 256K/8K optional profile improves 21K prefill 3.8%. Runbook: [`serving/docs/DS4-ON-THOR.md`](serving/docs/DS4-ON-THOR.md). |
