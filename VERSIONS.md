...
|---|---|---|
| **Qwen3.8 Flash-Next fused GDN + PLE SM110 overlay** | **verified serving (2026-09-05)** | `nemoclaw-thor/qwen38-flash-next-vllm:sm110-gdn-cuda`, over `blazux/qwen3.8-Flash-DGX@4b723de2e2c465d866738b57ae64bde6e8c07744` and the FlashInfer CUTLASS MoE SM110 gate. Standalone fused GDN source pinned to `082cf021b7ef96e4819e386846ea34e5ef21c655`, built for SM110a; 22 upstream MTP tests passed. Random-advised, parallel PLE reads preserve checkpoint bytes. Existing local RadixArk snapshot `7b719225242aacd3dbd3f9407468c2ee9a9d2594`, BF16 KV, 0.90 memory, 256K context and four scheduler slots. Build/launch/results: [`serving/docs/QWEN38-FLASH-NEXT-FAST-THOR.md`](serving/docs/QWEN38-FLASH-NEXT-FAST-THOR.md). Separate from the default serving image/profile. |
| **Qwen3.8 DSpark SM110 tuning overlay** | **prepared; runtime validation pending** | Thin local image `nemoclaw-thor/vllm-openai:qwen38-dspark-sm110` layered on the verified Qwen3.8 Thor image. Profile `qwen3.8-27b-nvfp4-dspark` uses local/cache-first `RadixArk/Qwen3.8-27B-NVFP4` + the existing no-copy Qwen architecture compatibility view of matched DSpark v2, K=7 with `dspark_draft_topk=512`, FP8 target/draft KV, 16K batched tokens, four sequences and 256K context. Startup autotuning covers NVFP4 verification widths 9/18/27/36 to avoid the observed width-9 `fp4_gemm` fallback. Recipe: [`serving/docs/QWEN38-27B-NVFP4-THOR.md`](serving/docs/QWEN38-27B-NVFP4-THOR.md#experimental-dspark-speed-profile). |
| **Qwen3.8 Thor SM110 overlay** | **verified working (2026-08-14); cache metadata refreshed 2026-08-20** | Profile-specific image `nemoclaw-thor/vllm-openai:qwen38-thor-sm110`: digest-pinned `vllm-openai:qwen38` preview, `v0.27.1` SM110 native extension, and official FlashInfer cubin 0.6.17. Verified with `unsloth/Qwen3.8-27B-NVFP4`, vision, FP8 KV, and BF16 MTP K=3. Model resolution is cache-first without a default revision pin; `THOR_HF_MODE=latest` forces an upstream refresh. The republished upstream snapshot retains the verified target/MTP tensor hashes and corrects metadata. This is intentionally separate from the default v9.1 image. Recipe: [`serving/docs/QWEN38-27B-NVFP4-THOR.md`](serving/docs/QWEN38-27B-NVFP4-THOR.md). |
