...
MCP_ENDPOINT="${MANYFORGE_MCP_ENDPOINT:-http://host.openshell.internal:9000/api/mcp}"
MCP_CONFIG_JSON=$(cat <<JSON
{"command":"python3","args":["${MCP_BRIDGE_PATH}"],"env":{"MANYFORGE_ENDPOINT":"${MCP_ENDPOINT}"}}
JSON
)
"${KEX_USER[@]}" "openclaw mcp set manyforge '${MCP_CONFIG_JSON}'" >/dev/null
ok "MCP server 'manyforge' registered"
"${KEX_USER[@]}" "openclaw mcp show manyforge" 2>&1 | sed 's/^/    /'

step "Composer reachability check"
if curl -fsS -o /dev/null --max-time 3 "${MCP_ENDPOINT}" 2>/dev/null; then
  ok "composer ${MCP_ENDPOINT} reachable from this host"
else
  printf '  ! composer endpoint %s did not answer from this host (curl).\n' "${MCP_ENDPOINT}"
  printf '  ! Inside the sandbox the same probe will use the network policy. If the\n'
  printf '  ! composer is bound to 127.0.0.1 only, it will not reach. Confirm with:\n'
  printf '  !   ss -tlnp | grep :9000\n'
  printf '  ! and rebind via demo-assistant-known-good.sh start_composer with --host 0.0.0.0.\n'
fi

step "Sandbox-side reachability probe (manyforge-composer policy)"
"${KEX_USER[@]}" "curl -fsS --max-time 5 -X POST -H 'Content-Type: application/json' -d '{\"jsonrpc\":\"2.0\",\"id\":\"probe\",\"method\":\"tools/list\"}' '${MCP_ENDPOINT}' | head -c 200" 2>&1 | sed 's/^/    /'
