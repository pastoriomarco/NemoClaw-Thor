...
| [`DS4-DEEPSEEK-V4-FLASH-INVESTIGATION.md`](serving/docs/DS4-DEEPSEEK-V4-FLASH-INVESTIGATION.md) | Landed (opt-in; 256K validated) | Dockerized Entrpi DS4 v0.5.1 with the 0731 base + matching DSpark drafter persists weights at `~/thor-hf-cache/ds4/`. On Thor `sm_110`, disabling v0.5.1's faulty streaming top-512 indexer via its built-in `DS4_CUDA_NO_TOPK_STREAM` escape eliminates the reproducible Xid 13; exact-needle requests passed at 84,797, 126,215, and 247,065 prompt tokens in a 262,144-token window. Runbook: [`DS4-ON-THOR.md`](serving/docs/DS4-ON-THOR.md). |
