...
| 8 | `nemotron3-nano-omni-30b-a3b-nvfp4` (NVIDIA tool-call recipe) | **80 ★★★★** | **87.7%** | ~11.7 | NVIDIA Nemotron 3 Nano Omni (released 2026-04-28) at T=0.6/top_p=0.95/max_tok=512. **+13 vs text-only Nano** — multimodal pretraining shifted agentic up. Native vision + audio + video |
| 9 | `nemotron3-nano-omni-30b-a3b-nvfp4` (T=0/think=false) | 75 ★★★★ | 86.3% | ~11.7 | Same model, deterministic regime. **−5 TEB vs primary** — opposite pattern from Qwen3.6: tool-call-specific vendor sampling matters when explicitly labeled "Tool calling" |
| 10 | `nemotron3-nano-30b-a3b-nvfp4` | 67 ★★★ | **85.6%** | 16.7 | NVIDIA's Dec 2025 text-only agentic flagship; mid-pack on TEB; **profile removed 2026-04-28** (superseded by Omni) |
...
3. **Nemotron 3 Nano (NVIDIA's Dec 2025 text-only agentic flagship) lands mid-pack at 67/100.** Strong IFEval (85.6%) but weaker on tool-eval-bench than the Qwen3.6-35B family. Possible drivers: small active-param count (3B vs Qwen 35B), our reliability tuning (no spec decode, max_seqs=2), no temperature tuning (NVIDIA recommends T=0.6/top_p=0.95 for tool calls; bench used defaults). **Profile removed and superseded by Nemotron 3 Nano Omni** (see #3a below).

3a. **Nemotron 3 Nano Omni (multimodal variant, released 2026-04-28) lifts the family agentic score by +13 to 80/100.** Same architecture class as the text-only Nano (30B-A3B hybrid Mamba MoE) but with vision/video/audio encoders + multimodal pretraining. At NVIDIA's vendor-recommended tool-call sampling (T=0.6, top_p=0.95, max_tokens=512) it scored TEB 80 / IFEval 87.7%. At T=0/think=false it scored TEB 75 / IFEval 86.3% — **vendor sampling won by +5 TEB, the OPPOSITE pattern from Qwen3.6**. Lesson: tool-call-specific vendor recommendations matter when they're explicitly labeled "Tool calling" in the model card / cookbook. Lands essentially tied with Cosmos-Reason2-8B (81) on agentic but adds native audio + better video / OCR. Still 13 below Qwen3.6-MTP winner — for max correctness the Qwen3.6 profile remains the choice; Omni becomes the candidate when single-endpoint multimodality (incl. voice) is required. **Boot needed `--kernel-config '{"enable_flashinfer_autotune": false}'` to dodge a v7-image cuDNN packaging bug** (apt 9.21.1 + pip 9.20.0 sublib version mismatch). v8 image should drop the apt cuDNN.
