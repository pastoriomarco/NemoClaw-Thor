...
> **Replaces** the prior `HERMES-MIGRATION-ANALYSIS-2026-06-02` analysis (deprecated and removed; never committed).
...
>
> **AD INTERIM 2026-06-12 — plan paused by operator decision.** Phases 0-3
> are complete; Phase 4 passed its per-turn floor (Hermes 54/66 = 81.8%,
> tops the per-turn board) but the **longitudinal gate has not been run**;
> Phase 5 is an interim decision (default stays `openclaw` + gemma-QAT).
> Exactly two gating items remain before the plan can be closed:
> (1) the apples-to-apples Hermes per-turn rerun (post `catalog_read`
> local-serve fix, corrected scorer); (2) the Phase-4 longitudinal run.
> Work resumes when those measurements are scheduled. See
...
4. **`manyforge_specs` is authoritative; respect it before changing anything in the manyforge tree.** The composer, the MCP bridge, the bridge envelope (`manyforge.assistant.provider_request.v0`), the catalog hash discipline, the policy contracts — these are specs-driven, not free-form. Before modifying any file under `dev_ws/src/manyforge/`, check the relevant section of `dev_ws/src/manyforge_specs/` (private `manyforge_specs` repo) — particularly `docs/cross-workspace-conventions.md` (private `manyforge_specs` repo), `docs/agent-playbook.md` (private `manyforge_specs` repo), and any ADR / kernel / port spec touching the file in question. If the refactor REQUIRES a spec change (e.g. moving the bridge code introduces a new contract), the spec change must land first, with explicit justification, and reviewed before any implementation work touches the dependent code. Silent drift from specs is the failure mode this principle blocks.
...
- **OpenClaw 2026.5.6 introduced the native tool-search shim** as a baked-in feature (`dist/pi-tools-iVT6BGHc.js`:1014–1063 in 2026.5.22). It is **not toggleable** — `tools.toolSearch=false` in `openclaw.json` is silently ignored. NemoClaw's `patch-openclaw-tool-catalog.js` is a no-op on 2026.5.6+ (`scripts/patch-openclaw-tool-catalog.js:244`).
...
- **The bundled NemoClaw `nemoclaw` plugin owns `api.registerProvider({id:"inference"})` unconditionally** (`nemoclaw plugin index.js:178`). Plugin-side `normalizeToolSchemas` overrides collide with no upstream `extendProvider` API. The right answer is not a plugin; it is the skill rewrite.
...
1. **Provider registration.** Composer's `build_assistant_provider()` only knows `nemoclaw` and `openclaw` ([assistant_provider.py:614](https://github.com/pastoriomarco/manyforge/blob/main/manyforge_composer/backend/assistant_provider.py#L614)); the launcher's routing in [assistant.sh:74](https://github.com/pastoriomarco/manyforge/blob/main/scripts/lib/assistant.sh#L74) falls back to Direct for unknown providers. Phase 1 adds an explicit lane registry — a `LANE_REGISTRY` dict in both files keyed by provider id with `(bridge_endpoint, default_port, transport_class)` per entry. The `hermes` provider id lands here.
2. **NemoClaw config emission.** `hermes-config.ts` currently emits no `mcp_servers` block. We add a fork or NemoClaw-Thor overlay that emits:
   ...
   **Do not add `host.openshell.internal` to `NO_PROXY`.** The bridge documents this at ``manyforge-mcp-bridge.py:75-81``: "the only network path to Composer from inside the sandbox runs through the proxy at 10.200.0.1:3128 — bypassing it yields 'Connection refused'." Match the OpenClaw provisioner pattern — proxy stays enabled for `host.openshell.internal`; `NO_PROXY` covers loopback hosts only.
...
└── docs/
    ...
    ├── LANE-COMPARISON.md  # extended to three lanes
...
- **Add explicit lane registry to Composer + launcher.** Today Composer's `build_assistant_provider()` ([assistant_provider.py:614](https://github.com/pastoriomarco/manyforge/blob/main/manyforge_composer/backend/assistant_provider.py#L614)) only knows `nemoclaw`/`openclaw`; the launcher's [assistant.sh:74](https://github.com/pastoriomarco/manyforge/blob/main/scripts/lib/assistant.sh#L74) falls back to Direct for unknown providers. Introduce `LANE_REGISTRY` keyed by provider id with `(bridge_endpoint, default_port, transport_class)` entries for `direct`, `openclaw`, and `hermes` (Hermes entry inert until Phase 4 wires it). Eliminates the "unknown provider falls back silently" foot-gun before bridge code moves.
...
- Merge `policies/manyforge-composer.preset.yaml` into `policies/manyforge-composer.merged.yaml` (single applied file containing both endpoints and the binary subject whitelist incl. `/usr/bin/python3.13`). Retain the `egress-shared` + `<lane>.overlay` files as documentation of the conceptual split — per §4.6 rev. 3, `policy-add` does not additively merge same-keyed blocks, so applying them separately drops the endpoints.
  ...
  - **Replace hardcoded `CONVERSATION_ID` constant** ([line 115](https://github.com/pastoriomarco/manyforge/blob/main/scripts/manyforge-mcp-bridge.py#L115)): currently `f"openclaw-{uuid...}"`; lane-prefix instead.
  - **Replace hardcoded `request_id` prefix** ([line 421](https://github.com/pastoriomarco/manyforge/blob/main/scripts/manyforge-mcp-bridge.py#L421)): currently `f"openclaw-{int(time...)}-{uuid...}"`; lane-prefix instead.
