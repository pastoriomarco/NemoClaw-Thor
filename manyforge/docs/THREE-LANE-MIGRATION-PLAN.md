...
**Gate.** All 10 lane probes PASS, OR specific failures documented with diagnosis (e.g. "probe 3 failed because NemoClaw bumped lkg and dropped /compact"). THREE-LANE plan landed and HERMES doc deprecated; plugin artifacts archived (not deleted). **Documentation deliverable**: `docs/PHASE-0-LANE-BASELINE.md` recording probe results, exact version tuple `(OpenClaw, Hermes, NemoClaw, vLLM, model, proxy-profile)`, and any flagged regressions. **Go = proceed to Phase 0.5. No-go = re-investigate the failed probes before any refactor.**
