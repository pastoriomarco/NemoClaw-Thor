...
| Bring-up | [`../../scripts/setup-hermes.sh`](../../../manyforge/scripts/setup-hermes.sh) (dev_ws) | The spike's strict order-of-ops: onboard → policy-add → cp bridge → inject `mcp_servers`+`base_url`+`API_SERVER_KEY` **before** gateway → `recover` (not `rebuild`) → verify subprocess + catalog. |
