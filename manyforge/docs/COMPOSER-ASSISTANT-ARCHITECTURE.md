...
|------|-------------|-------|-----------|-----|-------------|-----------------|------------------|---------------|-----------------|
| `qwen3.8-27b-nvfp4` | `unsloth/Qwen3.8-27B-NVFP4` (cache-first) | mixed NVFP4 W4A4 + protected FP8/BF16; FP8 KV | ~24 GB weights; ~21.2 GB profile image | 262 144 | `qwen3_coder` | `qwen3` | on | live API/vision/MTP smoke; ~22.5 tok/s short-request decode | Coding/architecture candidate. Custom digest-pinned Qwen3.8 + SM110 image; MTP K=3. `THOR_HF_MODE=latest` forces an upstream model refresh. See [`QWEN38-27B-NVFP4-THOR.md`](../../serving/docs/QWEN38-27B-NVFP4-THOR.md). |
