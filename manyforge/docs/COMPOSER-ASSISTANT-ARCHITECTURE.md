...
| `OPENCLAW_ASSISTANT_USE_GATEWAY` | `false` | When `true`, dispatch via the in-sandbox **persistent gateway** (HTTP at `:18789`). When `false`, shell-out to `openclaw` CLI per request. | **`false` in production** (reverted 2026-06-03). OpenClaw 2026.5.22 does not expose `/v1/chat/completions` on the gateway HTTP server — only `/v1/responses`, `/v1/embeddings`, `/v1/models`, `/v1/token` — so `gateway_http` transport returns 404 for every call. CLI shell-out is the only working path until OpenClaw exposes `/v1/chat/completions` or the bridge migrates to `/v1/responses`. Set to `true` only when running an OpenClaw build that has the endpoint, or testing the legacy path explicitly. |
...
|------|-------------|-------|-----------|-----|-------------|-----------------|------------------|---------------|-----------------|
| `qwen3.8-27b-nvfp4` | `unsloth/Qwen3.8-27B-NVFP4` (cache-first) | mixed NVFP4 W4A4 + protected FP8/BF16; FP8 KV | ~24 GB weights; ~21.2 GB profile image | 262 144 | `qwen3_coder` | `qwen3` | on | live API/vision/MTP smoke; ~22.5 tok/s short-request decode | Coding/architecture candidate. Custom digest-pinned Qwen3.8 + SM110 image; MTP K=3 and seven scheduler slots. `THOR_HF_MODE=latest` forces an upstream model refresh. See [`QWEN38-27B-NVFP4-THOR.md`](../../serving/docs/QWEN38-27B-NVFP4-THOR.md). |
