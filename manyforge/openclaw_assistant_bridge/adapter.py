    ...
    *,
    ...
    rules_block = "\n".join(
        [
            ...
            "`tree_draft_insert_node` → `{parentName, node: {id, name, "
            "params}, position: {afterName} | {beforeName} | {index}}` "
            "(NOT `parent_name`/`node_id`/`after_name`); "
            "`tree_draft_update_node_params` → `{nodeName, params, "
            "merge}` (NOT `node_name`); "
            "`tree_draft_delete_node` → `{nodeName}`; "
            "`tree_draft_move_node` → `{nodeName, newParentName, "
            "position}`; "
            "`tree_draft_change_node_kind` → `{nodeName, newKind, "
            "params}`; "
            "`scene_draft_add_object` → `{name, shape: {type, "
            "box_dims}, pose: {position, orientation_quat}}`; "
            ...
            "loops; if the validator rejected your previous call with "
            "\"missing required field <X>\", look at the camelCase "
            "form of <X> and the nested shape from the schema before "
            "retrying.",
