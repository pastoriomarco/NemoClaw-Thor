...
_PRIMER_CODE_MODE = (
    ...
    "they are dispatched from inside the JavaScript body of "
    "`tool_search_code({code: \"<body>\"})`. The body has access to "
    "a `tools` namespace exposing THREE control verbs PLUS auto-"
    "bound named convenience entries: "
    "(a) `await tools.call(\"<tool_name>\", {...args})` — universal "
    "explicit dispatch; works for every tool. "
    "(b) `await tools.<tool_name>({...args})` — convenience shorthand "
    "auto-bound at runtime, equivalent to (a). Only works when "
    "<tool_name> is a valid JS identifier and unique in the catalog "
    "(true for every manyforge tool). "
    "(c) `await tools.search(query, opts)` / "
    "`await tools.describe(id)` — discovery. Use only when the "
    "tool name is unknown to you. "
    "Most efficient pattern: `tool_search_code({code: \"const r = "
    "await tools.tree_draft_wrap_node({...}); return r;\"})`. "
    "If the named convenience binding ever fails (e.g. on a tool "
    "your build has shadowed), fall back to "
    "`await tools.call(\"<name>\", input)`. "
    "Always `await` real tool calls; always `return` the result so "
    "OpenClaw forwards it to you. Argument shapes inside the body "
    "are camelCase + nested objects (Rule 8a). Do NOT call real "
    "tools as if they were top-level entries in tools[] — they are "
    "NOT, the gateway will refuse, and your tools[] genuinely shows "
    "only `tool_search_code`. Do NOT say \"the tool isn't available\""
    " — it IS, via the tools namespace inside tool_search_code."
)
